...
> [!WARNING]
> **本项目目前是 0.1.3 实验版。** 现阶段只完成了自动化测试，以及 Windows / Obsidian 1.13.7 下的基础连接和界面验证；尚未经过真实课程中的长期学习、复习效果和公开用户测试。请勿把它作为重要学习记录的唯一副本。
>
> **本项目尚未提交或发布到 Obsidian Community Plugins。** 目前无法在 Obsidian 的社区插件市场中搜索安装；GitHub Release 仅提供实验版手动安装包，不代表 Obsidian 官方审核或收录。

当前版本仅支持桌面端、单人使用。基础聊天与学习记录已实现；完整的长期个性化指导仍在规划中。

项目的长期需求、科学学习内核、技术边界和后续协作规则统一记录在 [个人学习系统与项目长期上下文](docs/个人学习系统与项目长期上下文.md)。
...
- **更快的首次与连续问答**：Obsidian 启动、切换笔记或更新选区后，后台连接 Codex 并提前创建当前笔记线程；同一笔记会话继续复用线程。
- **按需发送资料**：问候不附带笔记；优先发送选区，普通问题发送相关片段，明确要求总结全文时才发送整篇笔记。
- **三个明确模式**：右栏可直接切换“快速”“均衡”和“深入”；依次固定使用 GPT-5.6 Luna `max`、GPT-5.6 Sol `medium` 和 GPT-5.6 Sol `high`，连接设置会检查三组配置是否可用。
...
- 提问时根据问题自动选择资料范围：选区最多 8,000 字符，相关笔记片段最多 8,000 字符；只有明确要求总结整篇笔记时才发送最多 20,000 字符。
- 同一笔记会话在插件本次运行期间复用 Codex 线程，并尽量在后台提前创建。首次建立或插件重启后恢复会话时，最多附带近期 8 条消息、每条最多 4,000 字符；连续追问不再重复附带这段历史。
...
5. **连续性与速度**：在现有会话复用、当前笔记线程预热和按需取资料基础上，继续研究跨重启长期摘要，并根据实际延迟调整三档模式。
...
当前 28 项单元测试覆盖学习事件、复习安排、上下文选择、回答模式、线程预热与复用、RPC 错误、停止与并发处理、事件幂等性及历史存储兼容性。
...
源代码遵循 [MIT License](LICENSE)。Obsidian、ChatGPT、Codex、Claude 等名称属于各自权利人；本项目为独立个人项目。
