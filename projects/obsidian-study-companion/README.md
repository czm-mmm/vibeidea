...
> [!WARNING]
> **本项目目前是 0.1.2 实验版。** 现阶段只完成了自动化测试，以及 Windows / Obsidian 1.13.7 下的基础连接和界面验证；尚未经过真实课程中的长期学习、复习效果和公开用户测试。请勿把它作为重要学习记录的唯一副本。
>
...
- **两个明确模式**：右栏可直接切换“快速”和“深入”；快速固定使用 GPT-5.6 Luna `max`，深入固定使用 GPT-5.6 Sol `high`，连接设置会检查这两组配置是否可用。
...
3. 如果未自动找到 Codex，填写其可执行文件路径；选择“快速 · Luna max”或“深入 · Sol high”，再检查连接与模式。
...
5. **连续性与速度**：在现有会话复用、当前笔记线程预热和按需取资料基础上，继续研究跨重启长期摘要，并根据实际延迟调整两档模式。
