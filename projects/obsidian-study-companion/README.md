...
> [!WARNING]
> **本项目目前是 0.1.1 实验版。** 现阶段只完成了自动化测试，以及 Windows / Obsidian 1.13.7 下的基础连接和界面验证；尚未经过真实课程中的长期学习、复习效果和公开用户测试。请勿把它作为重要学习记录的唯一副本。
>
...
- **更快的连续问答**：Obsidian 启动后后台预热连接；同一笔记会话复用 Codex 线程，并显示连接、思考与生成阶段。
- **按需发送资料**：问候不附带笔记；优先发送选区，普通问题发送相关片段，明确要求总结全文时才发送整篇笔记。
- **模型来自 Codex**：连接设置动态列出当前账号可用模型，并只显示该模型支持的推理强度。
...
3. 如果未自动找到 Codex，填写其可执行文件路径；模型与推理强度直接从当前 Codex 返回的选项中选择，留空时跟随 Codex 默认设置。
...
- 同一笔记会话在插件本次运行期间复用 Codex 线程。首次建立或插件重启后恢复会话时，最多附带近期 8 条消息、每条最多 4,000 字符；连续追问不再重复附带这段历史。
...
5. **连续性与速度**：在现有会话复用和按需取资料基础上，继续研究跨重启长期摘要与自动选择轻量／深入模型。
...
当前 26 项单元测试覆盖学习事件、复习安排、上下文选择、线程复用、RPC 错误、停止与并发处理、事件幂等性及历史存储兼容性。
