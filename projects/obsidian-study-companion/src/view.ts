...
export class StudyView extends ItemView {
  ...
  status(text:string){this.message=text;const current=this.contentEl.querySelector<HTMLElement>('.sc-status');if(current){current.setText(text);current.hidden=!text;}else this.render();}
  render(followLatest=false){
    ...
    if(session){
      ...
      label(preview,'提问时优先发送选区或相关片段；明确要求总结全文时才发送整篇笔记。');
    ...
    const status=label(footer,this.message,'sc-status');status.hidden=!this.message;status.setAttribute('role','status');
  ...
  private renderChat(body:HTMLElement,footer:HTMLElement,session?:Session){
    ...
    const actions=footer.createDiv({cls:'sc-actions sc-compose-actions'});const scope=label(actions,'资料：'+prepareContext(session.context,session.questionDraft).label,'sc-keyhint');scope.title='Enter 发送；Shift + Enter 换行';
    input.addEventListener('input',()=>scope.setText('资料：'+prepareContext(session.context,input.value).label));
  ...
  private chatPrompt(session:Session,question:string,includeRules=true,includeHistory=true){
    const parts:string[]=[];if(includeRules)parts.push(TUTOR_RULES);
    parts.push('本轮资料（JSON）：\n'+sourceContext(session.context,question));
    if(includeHistory)parts.push('最近对话（JSON）：\n'+JSON.stringify(session.messages.filter(m=>m.status==='complete').slice(-8).map(m=>({role:m.role,text:m.text.slice(0,4000)}))));
  ...
  private async sendChat(){
    ...
    const bridge=this.plugin.getBridge();const prompt=this.chatPrompt(session,question,false,!bridge.hasConversation(session.context.id));
    ...
    try{
      await this.plugin.persist();session.questionDraft='';this.message='正在准备上下文…';this.render(true);
      this.contentEl.querySelector<HTMLTextAreaElement>('.sc-composer')?.focus({preventScroll:true});
      answer.text=await bridge.answer(prompt,text=>{
        answer.text=text;
        for(const view of this.plugin.views()){
          const target=view.contentEl.querySelector(`[data-message-id="${answer.id}"]`);
          if(target)target.textContent=text;
        }
      },{conversationId:session.context.id,onStage:text=>this.plugin.views().forEach(view=>view.status(text))});answer.status='complete';this.message='';
    }catch(error){answer.status='failed';if(!session.questionDraft.trim())session.questionDraft=question;this.message=error instanceof Error?error.message:String(error);this.plugin.views().forEach(view=>view.status(this.message));}
    finally{this.plugin.busy=false;await this.plugin.persist();if(answer.status==='complete')this.plugin.views().forEach(view=>view.status(''));this.plugin.redraw();}
  }
