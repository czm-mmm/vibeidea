...
export interface CodexModel {
  model:string;displayName:string;description:string;isDefault:boolean;defaultReasoningEffort?:string;
  supportedReasoningEfforts:{reasoningEffort:string;description?:string}[];
}
export interface AnswerOptions {conversationId?:string;onStage?:(text:string)=>void}
...
export class CodexBridge {
  ...
  constructor(private binary:string,private model:string='',private effort:string=''){}
  async connect(refreshAccount=false){
    if(this.starting)await this.starting;
    else if(!this.rpc){this.starting=this.start().finally(()=>{this.starting=undefined;});await this.starting;}
    if(refreshAccount||!this.account.connected)return this.readAccount();
    return this.account;
  }
  private async start(){
  ...
  async listModels(refresh=false):Promise<CodexModel[]>{
    await this.connect();if(this.models&&!refresh)return this.models;
    const value=await this.rpc!.request('model/list',{});
    const rows=Array.isArray(value)?value:Array.isArray(value?.data)?value.data:Array.isArray(value?.models)?value.models:[];
    const models:CodexModel[]=rows.filter((row:any)=>typeof row?.model==='string').map((row:any)=>({
      model:row.model,displayName:typeof row.displayName==='string'?row.displayName:row.model,
      description:typeof row.description==='string'?row.description:'',isDefault:row.isDefault===true,
      defaultReasoningEffort:typeof row.defaultReasoningEffort==='string'?row.defaultReasoningEffort:undefined,
      supportedReasoningEfforts:(Array.isArray(row.supportedReasoningEfforts)?row.supportedReasoningEfforts:[]).map((item:any)=>typeof item==='string'?{reasoningEffort:item}:{reasoningEffort:item.reasoningEffort,description:item.description}).filter((item:any)=>typeof item.reasoningEffort==='string')
    }));
  ...
  async answer(prompt:string,onText:(text:string)=>void=()=>{},options:AnswerOptions={}):Promise<string>{
    ...
    this.responding=true;this.cancelRequested=false;options.onStage?.('正在连接 Codex…');try{return await this.runAnswer(prompt,onText,options);}finally{this.responding=false;}
  }
  private async runAnswer(prompt:string,onText:(text:string)=>void,options:AnswerOptions):Promise<string>{
    ...
    return new Promise<string>((resolve,reject)=>{
      let finished=false,streaming=false;
      ...
      const listener=(method:string,p:any)=>{
        ...
        if(method==='item/agentMessage/delta'){if(!streaming){streaming=true;options.onStage?.('正在生成回答…');}chunks.set(p.itemId,(chunks.get(p.itemId)||'')+p.delta);onText([...chunks.values()].join('\n\n'));}
      ...
      options.onStage?.('模型正在思考…');
      rpc.request('turn/start',{threadId,input:[{type:'text',text:prompt,text_elements:[]}],cwd:this.directory,approvalPolicy:'never',sandboxPolicy:{type:'readOnly',networkAccess:false},...(this.model?{model:this.model}:{}),...(this.effort?{effort:this.effort}:{})}).then(value=>{active.turnId=value.turn.id;if(active.cancelled)void this.interrupt();}).catch(error=>finish(error));
    });
  }
