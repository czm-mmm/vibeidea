...
test('同一学习会话连续提问只建立一次 Codex 线程',async()=>{
  const bridge:any=new CodexBridge('unused');bridge.directory='C:/temp';bridge.config={};bridge.connect=async()=>({connected:true});
  const listeners=new Set<(method:string,params:any)=>void>();let threadStarts=0,turns=0;
  bridge.rpc={listeners,request:async(method:string,params:any)=>{
    if(method==='thread/start'){threadStarts++;return {thread:{id:'thread-1'},sandbox:{type:'readOnly'}};}
    if(method==='turn/start'){turns++;setTimeout(()=>{for(const listener of listeners){listener('item/agentMessage/delta',{threadId:params.threadId,itemId:'message-'+turns,delta:'回答'+turns});listener('turn/completed',{threadId:params.threadId,turn:{status:'completed'}});}},0);return {turn:{id:'turn-'+turns}};}
    return {};
  }};
  assert.equal(await bridge.answer('第一问',()=>{},{conversationId:'课程一'}),'回答1');
