...
const plugin:any={app,data,events:[],questions:[q,{...q,id:'q2',title:'复合函数的方法辨别'}],busy:false,store:{appendUnderstanding:async()=>{}},session:()=>data.sessions[data.activeSession!],persist:async()=>{},views:()=>[view],redraw:()=>view.render(),record:async(e:any)=>{plugin.events.push(structuredClone(e));},reload:async()=>view.render(),getBridge:()=>({hasConversation:()=>false,interrupt:async()=>{},answer:async(_prompt:string,onText?:any)=>{onText?.('模型回答');return '模型回答';}}),report:(e:any)=>{(window as any).lastNotice=e.message;},capture:async()=>{},detail:async()=>{},prewarm:async()=>{},setMode:async(mode:'fast'|'balanced'|'deep')=>{data.settings.mode=mode;view.render();}};
