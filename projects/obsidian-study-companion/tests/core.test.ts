...
import { parseQuestion, parseEvent, serializeEvent, schedule, reviewQueue, validFolder, newContext, prepareContext, sourceContext, type StudyEvent, type Question, type Attempt } from '../src/core';
...
test('问候不发送笔记，选区优先于整篇笔记',()=>{
  const context=newContext('数学/a.md','a','数学','导数定义','正文'.repeat(5000));
  assert.equal(prepareContext(context,'你好').scope,'none');
  const selected=prepareContext(context,'这里为什么这样写');assert.equal(selected.scope,'selection');assert.equal(selected.payload.selection,'导数定义');
});
test('普通问题只选择相关片段，明确总结时才发送全文',()=>{
  const text='# 指针\n\n指针保存内存地址。\n\n# 数组\n\n数组包含连续元素。\n\n# 函数\n\n函数可以接收参数。';
  const context=newContext('C语言.md','C语言','C语言','',text);
  const relevant=prepareContext(context,'指针保存的是什么');assert.equal(relevant.scope,'relevant');assert.match(String(relevant.payload.noteText),/内存地址/);
  assert.equal(prepareContext(context,'总结整篇笔记').scope,'full');
});
