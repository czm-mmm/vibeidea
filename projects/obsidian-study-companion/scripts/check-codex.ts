...
(async()=>{try {
  ...
  if(process.argv.includes('--models'))console.log(JSON.stringify({models:(await bridge.listModels()).map(model=>({model:model.model,displayName:model.displayName,isDefault:model.isDefault,efforts:model.supportedReasoningEfforts.map(item=>item.reasoningEffort)}))}));
